...
ICE_CREATE {
	ICE_Game game = ICE_CreateGame("ICE : Pong", 1280, 720); ICE_GameResizable(&game, ICE_False);
	DATA0 *data = ICE_AddData(&game, sizeof(DATA0));

	ICE_CreateEntityManager(&game);
	ICE_CreateEntity(&game, 0);
	ICE_CreateEntity(&game, 0);
	ICE_CreateEntity(&game, 0);

	// Player
	ICE_SetEntitySize(&game, 0, 0, 20, 70);
	ICE_SetEntityPosition(&game,0,0,20,game.camera.h/2-35);
	data->player.speed_max = 500;
	
	// Ennemy
	ICE_SetEntitySize(&game, 0, 1, 20, 70);
	ICE_SetEntityPosition(&game, 0, 1, game.camera.w - 40, game.camera.h / 2 - 35);
	data->ennemy.speed_max = 500;
	
	// Ball
	ICE_SetEntitySize(&game, 0, 2, 20, 20);
	ICE_SetEntityPosition(&game, 0, 2, game.camera.w / 2 - 10, game.camera.h / 2 - 10);
	data->ball.speed = 150;
...
void Collision_Ball(ICE_Game *game)
{
	...
	ICE_Rect ballrect = ICE_GetRectFromEntity(game, 0, 2);
...
void Player_Movement(ICE_Game *game){
	...
	ICE_ShiftEntityPosition(game, 0, 0, 0, data->player.speed);
	ICE_ShiftEntityPosition(game, 0, 1, 0, data->ennemy.speed);
	ICE_ShiftEntityPosition(game, 0, 2, data->ball.dx*data->ball.speed, data->ball.dy*data->ball.speed);
}

ICE_UPDATE {
	Player_Movement(game);
	Collision_Ball(game);
	ICE_DrawRectangleFill(game, ICE_GetRectFromEntity(game, 0,0), NewColor(255, 255, 255)); // Player
	ICE_DrawRectangleFill(game, ICE_GetRectFromEntity(game, 0, 1), NewColor(255, 255, 255)); // Ennemy
	ICE_DrawRectangleFill(game, ICE_GetRectFromEntity(game, 0, 2), NewColor(255, 255, 255)); // Ball
