...
void iceMathsMoveFromAngle(iceFloat *x, iceFloat *y, const iceFloat angle, const iceFloat r) {
	*x += r * cos(angle); *y += r * sin(angle);
}

/// Calculate the angle for two point in degree
iceFloat iceMathsAngleCalculatDegree(const iceFloat x1, const iceFloat y1, const iceFloat x2, const iceFloat y2) {
	iceFloat xdif = x2 - x1; iceFloat ydif = y2 - y1;
	return 57.29*atan2(ydif, xdif);
}

/// Calculate the angle for two point in radiant
iceFloat iceMathsAngleCalculatRadiant(const iceFloat x1, const iceFloat y1, const iceFloat x2, const iceFloat y2) {
	iceFloat xdif = x2 - x1; iceFloat ydif = y2 - y1;
